...
local function is_comment_block_surround(filetype_config, lines)
   ...
   return (first_line:sub(1, #filetype_config['block']['left']) == filetype_config['block']['left']) and (last_line:sub(#last_line - #filetype_config['block']['right'] + 1, #last_line) == filetype_config['block']['right'])
end
...
local function uncomment_block_surround(filetype_config, context)
   ...
   context.lines[#context.lines] = context.lines[#context.lines]:sub(1, #context.lines[#context.lines] - #filetype_config['block']['right'] - 1)
...
local function is_comment_block_multiline(filetype_config, lines)
   local first_line = trim(lines[1])
   return (first_line:sub(1, #filetype_config['single']) == filetype_config['single'])
end
