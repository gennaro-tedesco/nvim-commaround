describe("testing block comments", function()
   ...
   local lua_ft_cfg, sql_ft_cfg, vim_ft_cfg, py_ft_cfg, html_ft_cfg, hs_ft_cfg
   ...
   setup(function()
    ...
    hs_ft_cfg = global_config.config['haskell']
	  html_ft_cfg = global_config.config['html']
	  hs_ft_cfg = global_config.config['haskell']
   end)
   ...
   it("is_comment_block_surround #surround", function()
	  ...
	  local html_lines = {" <!-- first line", "second line", "third line -->"}
	  assert.is_true(comments.is_comment_block_surround(html_ft_cfg, html_lines))
